...
//			}
//		}
//		NRF_LOG_INFO("Heart Rate : %d", beatAvg);
//}
...
static bool writeRegister(uint8_t regNumber, uint8_t value){
		...
		valueBuffer[1] = value;
		//int32_t toSend = (int32_t)value;
		uint32_t err_code = nrf_drv_twi_tx(&m_twi, 0x19, valueBuffer, sizeof(valueBuffer), false);
		nrf_delay_ms(10);
		return true;
}
...
static uint8_t readRegister(uint8_t regNumber){
		...
		//uint8_t resultsWhoAmI;
		uint8_t whoAmIPointer = regNumber;
		nrf_drv_twi_tx(&m_twi, 0x19, &whoAmIPointer, 1, true);
		nrf_delay_ms(10);
		nrf_drv_twi_rx(&m_twi, 0x19, &m_sample, 1);
		nrf_delay_ms(10);
		NRF_LOG_INFO("TWI 0x%x: 0x%x.", regNumber, m_sample);
		return m_sample;
}
...
static uint8_t readRegister_FIFO(uint8_t regNumber){
		...
		nrf_drv_twi_tx(&m_twi, 0x19, &whoAmIPointer, 1, true);
		nrf_delay_ms(10);
		nrf_drv_twi_rx(&m_twi, 0x19, m_sample_FIFO, 1);
		nrf_delay_ms(10);
		NRF_LOG_INFO("TWI 0x%x: 0x%x.", regNumber, m_sample_FIFO);
...
static void read_data()
{
    ...
    /* Read 1 byte from the specified address - skip 3 bits dedicated for fractional part of temperature. */
    ret_code_t err_code = nrf_drv_twi_rx(&m_twi, 0x19, &m_sample, 1);
    APP_ERROR_CHECK(err_code);
}
